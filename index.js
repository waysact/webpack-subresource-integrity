...
/**
 * Check if we need to ignore the SRI generation key for an asset.
 * @param {String} asset Asset's name
 * @return {Boolean}
 */
SubresourceIntegrityPlugin.prototype.isIgnoredAsset =
  function isIgnoredAsset(asset) {
    // regexp as the asset is generated by webpack based on the entry's name
    return this.options.ignored && this.options.ignored.test(asset);
  };
...
SubresourceIntegrityPlugin.prototype.addMissingIntegrityHashes =
  function addMissingIntegrityHashes(assets) {
    ...
    Object.keys(assets).forEach(function loop(assetKey) {
      ...
      if (!asset.integrity && !self.isIgnoredAsset(assetKey)) {
...
SubresourceIntegrityPlugin.prototype.processTag =
  function processTag(compilation, tag) {
    ...
    if (!Object.prototype.hasOwnProperty.call(tag.attributes, "integrity") && !this.isIgnoredAsset(src)) {
